...
- Ideal world: SashimiData S4 object
  ...
  - API interface for other components - it handles cache, conversions,
    etc.
  ...
  - Then
    ...
    only needs this object as input.
...
- TxDb can “intuit” the gene_name, flatExons, etc.
  ...
  - Default could use gene_id also as gene_name if no other option
    exists.
...
- `SplicejamGenes` S4 object?
  ...
  - Paths to derive:
    ...
    - txdb, tx2geneDF - sufficient to create: flatExonsByGene,
      flatExonsByTx
    - gtf - sufficient to create: tx2geneDF, txdb, flatExonsByGene,
      flatExonsByTx
