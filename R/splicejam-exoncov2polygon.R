...
#' @param gapWidth `integer` value, default 200 from
#'    `options('splicejam.gap')`, passed to `make_ref2compressed()` to set
#'    a fixed with for gap, thus compressing introns to a fixed visual
#'    size. When NULL, it uses 3.3 times the median feature length.
...
(gr,
 ...
 sample_id=NULL,
 baseline=NULL,
 gapWidth=getOption("splicejam.gap", 250),
 doPlot=FALSE,
 ...
 ...)
{
   ...
   if (TRUE) {
      covPolyL <- lapply(jamba::nameVector(covNames), function(iName){
         ## Check if this coverage column contains NumericList (fast path)
         cov_col_test <- GenomicRanges::values(gr)[[iName]][1];
         
         if (inherits(cov_col_test, "NumericList") && "fortify" %in% coord_style) {
            ## FAST PATH: NumericList input - use direct assignment with names(gr)
            ## Convert NumericList to list for efficient storage in data.frame
            ## This avoids the inner lapply and processes all exons at once
            cov_list <- as.list(GenomicRanges::values(gr)[[iName]]);
            xVals <- Map(seq.int, from=GenomicRanges::start(gr),
               to=GenomicRanges::end(gr))
            polyDF <- data.frame(check.names=FALSE,
               x=I(xVals),
               y=I(cov_list),
               x_start=GenomicRanges::start(gr),
               x_end=GenomicRanges::end(gr),
               cov=iName,
               gr=names(gr))
         } else {
            ## SLOW PATH: Non-NumericList input - use lapply for backward compatibility
            polyL <- lapply(jamba::nameVectorN(gr), function(iGRname){
               cov_col <- GenomicRanges::values(gr[iGRname])[[iName]];
               iBase <- baselineV[iGRname];
               
               yVals1a <- unlist(cov_col);
               ...
               xVals <- seq(from=GenomicRanges::start(gr[iGRname]),
               ...
               if (nrow(xy) > 0 && "fortify" %in% coord_style) {
                  xy <- data.frame(check.names=FALSE,
                     x=I(list(xy[, "x"])),
                     y=I(list(xy[, "y"])),
                     x_start=GenomicRanges::start(gr[iGRname]),
                     x_end=GenomicRanges::end(gr[iGRname]),
                     cov=iName,
                     gr=iGRname)
               }
               xy
            });
            polyDF <- jamba::rbindList(polyL);
         }
         polyDF
      });
   }
   ...
   if ("base" %in% coord_style) {
      ## Todo: Repair this step since covPolyL has changed
      ## to store only the coverage, not the baseline y-values and x-values
